...
## How to Test if the NQ Trading Strategy is Working

This guide provides comprehensive methods to validate that your NQ intraday trading strategy is functioning correctly and generating quality signals.
...
```bash
# Run complete validation suite (1 minute)
...
- ✅ Connection status (IB Gateway)
- ✅ Telegram integration

### Option 2: Individual Tests
```bash
# 1. Test all components (2 minutes)
./scripts/run_tests.sh

# 2. Test signal generation specifically (30 seconds)
python3 scripts/test_signal_generation.py

# 3. Test full service with mock data (2 minutes)
...
```python
# Edit scripts/test_signal_generation.py
...
1. **Confidence Scores**
   ...
   - Higher is better
   - Signals below 0.50 are filtered out
...
# Run specific test
pytest tests/test_nq_agent_signals.py::test_signal_generation -v
```
...
Create a test script to run the service for longer:

```python
# test_extended_service.py
import asyncio
...
async def test():
    provider = MockDataProvider(
        base_price=15000.0,
        volatility=75.0,  # Higher volatility for more signals
        trend=1.5,
    )
    ...
    await asyncio.wait_for(start_task, timeout=600.0)
    await service.stop()
...
# (Performance data is tracked in state directory)
```
...
2. Review confidence thresholds in config
...
**Solutions:**
1. Start IB Gateway: `./scripts/start_ibgateway_ibc.sh`
2. Check port: `netstat -tlnp | grep 4002`
3. Verify connection: `./scripts/check_gateway_status.sh`
...
### Test Commands
```bash
# Comprehensive validation (recommended first step)
python3 scripts/validate_strategy.py
...
### Key Files
- `scripts/test_signal_generation.py` - Test signal logic
- `scripts/test_nq_agent_with_mock.py` - Test full service
- `tests/test_nq_agent_signals.py` - Unit tests
...
- [Testing Guide](./TESTING_GUIDE.md) - Basic testing procedures
- [NQ Agent Guide](./NQ_AGENT_GUIDE.md) - Agent documentation
