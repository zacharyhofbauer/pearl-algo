...
3. **Review Overnight Activity:**
   ...
   - Review service state: `cat data/agent_state/<MARKET>/state.json | jq`
   - Check recent signals: `tail -20 data/agent_state/<MARKET>/signals.jsonl | jq`
...
```bash
tail -f data/agent_state/<MARKET>/signals.jsonl | jq
```
...
The watchdog is designed for cron/systemd timers (e.g., every 5 minutes). It reads `data/agent_state/<MARKET>/state.json` and returns non-zero exit codes for warning/critical conditions.
...
3. **Check for errors:**
   ...
   - Review service state: `cat data/agent_state/<MARKET>/state.json | jq .error_count`
...
3. **Check buffer size:**
   ```bash
   cat data/agent_state/<MARKET>/state.json | jq .buffer_size
   ```
...
1. **Check error count:**
   ```bash
   cat data/agent_state/<MARKET>/state.json | jq .error_count
   ```
...
4. **Check connection failures:**
   ```bash
   cat data/agent_state/<MARKET>/state.json | jq .connection_failures
   ```
...
**State & Data:**
- `data/agent_state/<MARKET>/state.json` - Current service state
- `data/agent_state/<MARKET>/signals.jsonl` - Signal history (JSONL format)
- `data/agent_state/<MARKET>/performance.json` - Performance metrics
