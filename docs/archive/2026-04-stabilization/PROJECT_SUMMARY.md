...
### 6. Current Platform Scope

The current tracked codebase is centered on:

- Market-agent orchestration in `src/pearlalgo/market_agent/`
- Tradovate execution in `src/pearlalgo/execution/tradovate/`
- API/web support in `src/pearlalgo/api/`
- Shared storage/utilities under `src/pearlalgo/storage/` and `src/pearlalgo/utils/`

Historical notes in this document may still mention retired subsystems from before
the revamp. Treat `docs/START_HERE.md`, `docs/PATH_TRUTH_TABLE.md`, and the dated
2026-04-01 audit docs as the current operational truth.
...
2. **Base config + optional overlay**:
   ...
   - Optional overlay: `PEARLALGO_CONFIG_PATH` (e.g., `config/accounts/tradovate_paper.yaml`)
...
**View Recent Signals**:
```bash
tail -20 data/agent_state/<MARKET>/signals.jsonl | jq
```
...
ls -la data/agent_state/<MARKET>/exports/
```
...
4. **Automated Execution (ATS)** - Disabled by Default:
   - Execution lives in `src/pearlalgo/execution/`
   - The older learning-layer paths referenced in earlier docs are no longer part of the current tracked tree
   - See `docs/ATS_ROLLOUT_GUIDE.md` as historical rollout context, not as the primary runtime map
...
- Eliminated the old dedicated state package and consolidated state handling into the active runtime modules
