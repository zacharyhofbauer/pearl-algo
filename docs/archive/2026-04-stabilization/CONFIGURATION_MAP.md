...
- `PEARL_API_KEY`
  ...
  - **Purpose**: Shared secret for API key auth (see `apps/pearl-algo-app` and `api_server.py`).
...
| `apps/pearl-algo-app/.env.local` | Web app API key + auth settings (auto-synced by `pearl.sh sync_env_local`: `NEXT_PUBLIC_API_KEY`, `PEARL_WEBAPP_AUTH_ENABLED`, `PEARL_WEBAPP_PASSCODE`). |
