 ...
 *
 * Handles read-only API key authentication for the Pearl Algo Web App.
 * Authentication is optional and controlled by:
 * - NEXT_PUBLIC_READONLY_API_KEY: preferred browser-safe API key
 * - NEXT_PUBLIC_API_KEY: legacy fallback name
 *
 * When a read-only browser key is set, requests will include the X-API-Key header.
 * When not set, requests are made without authentication (for local development).
 */

import { useOperatorStore } from '@/stores'

// Browser-safe key used for read-only API/WS access. Prefer the explicit name,
// but keep the legacy variable as a compatibility fallback during migration.
...
export function isAuthConfigured(): boolean {
  return Boolean(READ_ONLY_API_KEY)
}
