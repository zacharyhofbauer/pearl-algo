...
export function getWebSocketUrl(): string {
  ...
  if (account === 'tv_paper') {
    ...
    // Production: /tv_paper/ws routed by Cloudflare tunnel to port 8001
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:'
    return `${protocol}//${hostname}/tv_paper/ws`
  }
...
export function getWebSocketApiKey(): string {
  return process.env.NEXT_PUBLIC_READONLY_API_KEY || process.env.NEXT_PUBLIC_API_KEY || ''
}
