...
beforeEach(() => {
  ...
  delete process.env.NEXT_PUBLIC_READONLY_API_KEY
...
describe('useWebSocket hook', () => {
  ...
  describe('connection establishment', () => {
    ...
    it('should send auth message on open when the read-only API key is configured', () => {
      process.env.NEXT_PUBLIC_READONLY_API_KEY = 'test-api-key'
      renderHook(() => useWebSocket({ url: TEST_URL }))
      const ws = getLastInstance()
...
describe('getWebSocketApiKey', () => {
  ...
  it('should prefer NEXT_PUBLIC_READONLY_API_KEY when set', () => {
    process.env.NEXT_PUBLIC_READONLY_API_KEY = 'my-readonly-key'
    process.env.NEXT_PUBLIC_API_KEY = 'legacy-key'
    expect(getWebSocketApiKey()).toBe('my-readonly-key')
  })

  it('should fall back to NEXT_PUBLIC_API_KEY when the new name is unset', () => {
    process.env.NEXT_PUBLIC_API_KEY = 'legacy-key'
    expect(getWebSocketApiKey()).toBe('legacy-key')
  })

  it('should return empty string when no browser API key is set', () => {
