...
class TestSharpeRatio:
    ...
    def test_positive_returns(self):
        ...
        df = pd.DataFrame({
            "realized_pnl": [0.0, 100.0, 200.0, 300.0, 400.0]
        })
        result = compute_sharpe_ratio(df)
        assert result > 0.0
    ...
    def test_mixed_returns(self):
        ...
        df = pd.DataFrame({
            "realized_pnl": [0.0, 100.0, -50.0, 200.0, -100.0, 150.0]
        })
        result = compute_sharpe_ratio(df)
        assert not pd.isna(result)
    ...
    def test_with_nan_values(self):
        ...
        df = pd.DataFrame({
            "realized_pnl": [0.0, 100.0, None, 200.0, None, 300.0]
        })
        result = compute_sharpe_ratio(df)
        assert not pd.isna(result)
        assert isinstance(result, float)
...
class TestTradeStatistics:
    ...
    def test_win_rate_calculation(self):
        ...
        df = pd.DataFrame({
            "realized_pnl": [100.0, 50.0, -25.0, -50.0, 75.0]
        })
        result = compute_trade_statistics(df)
