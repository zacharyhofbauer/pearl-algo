...
def test_workflow_state_transitions(sample_state):
    ...
    # Initial state
    assert sample_state.current_step == "market_data" or sample_state.current_step is None
...
def test_error_handling_in_state(sample_state):
    ...
    assert sample_state.trading_enabled == True
    
    # Trigger kill switch
    sample_state.kill_switch_triggered = True
    assert sample_state.kill_switch_triggered == True
