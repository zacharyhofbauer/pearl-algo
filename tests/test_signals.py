...
def test_calculate_bollinger_bands():
    ...
    # Test with insufficient data
    upper_short, middle_short, lower_short = calculate_bollinger_bands(
        prices[:10], period=20
    )
...
def test_strategy_parameters():
    ...
    result = generate_signal(
        "ES", df, strategy_name="sr", fast=10, slow=30, tolerance=0.001
    )
    ...
    result = generate_signal(
        "ES", df, strategy_name="breakout", lookback=30, volume_multiplier=2.0
    )
    ...
    result = generate_signal(
        "ES", df, strategy_name="mean_reversion", bb_period=30, rsi_period=21
    )
