...
def test_sizing_vol_basic():
    size = volatility_position_size(
        account_equity=100000, risk_per_trade=0.01, atr=10, dollar_vol_per_point=50
    )
    assert size > 0
...
def test_pnl_tracker_daily_loss():
    ...
    tracker.record_fill(
        FillEvent(timestamp=now, symbol="ES", side="BUY", quantity=1, price=100)
    )
    tracker.record_fill(
        FillEvent(timestamp=now, symbol="ES", side="SELL", quantity=1, price=90)
    )
