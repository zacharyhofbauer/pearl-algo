"""
Tests for MFFU account detection and routing logic.

Covers:
- is_mffu_account: equity=0 (True), equity present (True), missing equity (False),
...
@pytest.mark.parametrize(
    "state_data, expected",
    [
        # MFFU with positive equity
        ({"tradovate_account": {"equity": 50000.0}}, True),
        # MFFU with equity = 0 (was False, now fixed to True)
        ({"tradovate_account": {"equity": 0}}, True),
        # MFFU with equity = 0.0
        ({"tradovate_account": {"equity": 0.0}}, True),
        # MFFU with negative equity (drawdown)
        ({"tradovate_account": {"equity": -500.0}}, True),
        # tradovate_account present but no equity key → not MFFU
        ({"tradovate_account": {"positions": []}}, False),
        # tradovate_account is empty dict → not MFFU
        ({"tradovate_account": {}}, False),
        # tradovate_account is None → not MFFU
        ({"tradovate_account": None}, False),
        # tradovate_account is a string (bad data) → not MFFU
        ({"tradovate_account": "invalid"}, False),
        # No tradovate_account key at all → not MFFU (Inception)
        ({"running": True}, False),
        # Empty state → not MFFU
        ({}, False),
    ],
    ids=[
        "equity_positive",
...
def test_is_mffu_account(tmp_path, state_data, expected):
    """Parametrized test for all edge cases of is_mffu_account."""
...
def test_is_mffu_account_corrupt_state(tmp_path):
...
def test_is_mffu_account_missing_state_file(tmp_path):
