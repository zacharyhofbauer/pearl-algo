"""
Tests for MFFUEvaluationTracker — MFFU 50K Rapid Evaluation challenge tracker.

Covers:
- MFFUEvalConfig creation with defaults
...
def _make_tracker(tmp_path: Path, **config_overrides) -> MFFUEvaluationTracker:
    """Create a tracker using tmp_path as the state directory."""
    cfg = MFFUEvalConfig(**config_overrides)
    return MFFUEvaluationTracker(config=cfg, state_dir=tmp_path)
...
class TestMFFUEvalConfig:
    """Tests for MFFUEvalConfig dataclass defaults."""
    ...
    def test_custom_overrides(self):
        cfg = MFFUEvalConfig(start_balance=100_000.0, profit_target=6_000.0)
...
class TestTrackerInit:
    """Tests for MFFUEvaluationTracker initialisation."""
