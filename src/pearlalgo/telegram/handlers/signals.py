"""
Signals handler: signal rejections (24h) and last signal decision.

Mirrors the dashboard Signals tab — the transparency/dev view.
"""
...
from pearlalgo.telegram.formatters.messages import format_signals_message, format_error_message
...
async def handle_signals(update: Any, context: Any) -> None:
    """Handle /signals command -- show signal rejections and last decision."""
    try:
        ...
        logger.error(f"Signals handler error: {e}", exc_info=True)
