...
Centralized configuration for Polygon API including API keys, URLs, rate limits, and retry settings.
"""
...
class PolygonConfig:
    ...
    def from_env(cls, api_key: Optional[str] = None) -> PolygonConfig:
        ...
        Args:
            api_key: API key (if None, reads from POLYGON_API_KEY env var)
        ...
        return cls(
            ...
            rate_limit_delay=float(os.getenv("POLYGON_RATE_LIMIT_DELAY", "0.25")),
            requests_per_minute=int(os.getenv("POLYGON_REQUESTS_PER_MINUTE", "200")),
            burst_limit=int(os.getenv("POLYGON_BURST_LIMIT", "10")),
            max_retries=int(os.getenv("POLYGON_MAX_RETRIES", "3")),
            initial_backoff=float(os.getenv("POLYGON_INITIAL_BACKOFF", "1.0")),
            max_backoff=float(os.getenv("POLYGON_MAX_BACKOFF", "60.0")),
            backoff_multiplier=float(os.getenv("POLYGON_BACKOFF_MULTIPLIER", "2.0")),
            circuit_breaker_failure_threshold=int(
                os.getenv("POLYGON_CIRCUIT_BREAKER_THRESHOLD", "5")
            ),
            circuit_breaker_recovery_timeout=float(
                os.getenv("POLYGON_CIRCUIT_BREAKER_TIMEOUT", "60.0")
            ),
