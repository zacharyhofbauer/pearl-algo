...
class TradeDataAccess:
    ...
    def format_for_context(
        ...
        Opens a SINGLE read-only connection for all sub-queries to avoid
        repeated open/close overhead (5 connections -> 1).
        ...
        try:
            with self._connection() as conn:
                ...
                if regime and any(word in query_lower for word in ["regime", "market", "trending", "ranging"]):
                    perf = self._query_regime_performance(conn, regime, days=30)
                ...
                if any(word in query_lower for word in ["long", "short", "direction", "side"]):
                    dir_perf = self._query_direction_performance(conn, days=30)
                ...
                if any(word in query_lower for word in ["similar", "pattern", "like this", "before"]):
                    ...
                    if direction:
                        similar = self._query_similar_trades(conn, direction, regime=regime, limit=3)
                        if similar:
                ...
                if any(word in query_lower for word in ["today", "week", "performance", "how am i"]):
                    summary = self._query_performance_summary(conn, days=7)
                ...
                if any(word in query_lower for word in ["hour", "morning", "afternoon", "time", "when"]):
                    hourly = self._query_hourly_performance(conn, days=30)
                    if hourly:
    ...
    def _query_similar_trades(self, conn, direction: str, regime: Optional[str] = None, limit: int = 5) -> List[Dict[str, Any]]:
        """Query similar trades using an existing connection."""
        try:
            if regime:
                cursor = conn.execute("""
                    SELECT * FROM trades
                    WHERE direction = ? AND regime = ?
                    ORDER BY exit_time DESC LIMIT ?
                """, (direction, regime, limit))
            else:
                cursor = conn.execute("""
                    SELECT * FROM trades
                    WHERE direction = ?
