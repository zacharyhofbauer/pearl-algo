...
def status_cmd(ctx: click.Context) -> None:
    ...
    console.print(Panel(gateway_table, title="🔌 IB Gateway", border_style="cyan" if gateway_ready else "red", box=box.ROUNDED))
    console.print()
    ...
    try:
        ...
        today_df = df[df["timestamp"].dt.strftime("%Y%m%d") == today] if "timestamp" in df.columns and not df.empty else df.iloc[0:0]
        ...
        risk_color = "green" if risk_state.status == "OK" else "yellow" if risk_state.status == "NEAR_LIMIT" else "red"
        risk_table.add_row("Status", f"[{risk_color}]{risk_state.status}[/{risk_color}]", "")
        risk_table.add_row("Remaining Buffer", f"${risk_state.remaining_loss_buffer:,.2f}", "")
        risk_table.add_row("Daily Loss Limit", f"${risk_state.daily_loss_limit:,.2f}", "")
        if risk_state.max_trades:
            remaining = max(0, risk_state.max_trades - trades_today)
            risk_table.add_row("Trades Today", f"{trades_today}/{risk_state.max_trades}", f"({remaining} remaining)")
        
        console.print(Panel(risk_table, title="⚠️  Risk State", border_style=risk_color, box=box.ROUNDED))
        console.print()
        ...
        perf_table.add_row("Today P&L", f"[{pnl_color}]${realized_pnl:,.2f}[/{pnl_color}]", "")
        perf_table.add_row("Total Trades", f"{len(df)}", "")
        perf_table.add_row("Trades Today", f"{trades_today}", "")
        
        console.print(Panel(perf_table, title="📊 Performance", border_style="cyan", box=box.ROUNDED))
        console.print()
