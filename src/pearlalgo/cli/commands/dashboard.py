...
@click.option(
    "--once", is_flag=True, help="Show dashboard once and exit (default: live updating)"
)
@click.option(
    "--refresh", type=int, default=5, help="Refresh interval in seconds (default: 5)"
)
@click.option(
    "--full",
    is_flag=True,
    help="Show comprehensive full-screen dashboard with more details",
)
@click.option("--menu", is_flag=True, help="Show interactive menu panel")
@click.option(
    "--interactive",
    is_flag=True,
    help="Interactive mode with menu and keyboard commands",
)
@click.pass_context
def dashboard_cmd(
    ctx: click.Context,
