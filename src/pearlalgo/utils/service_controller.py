...
class ServiceController:
    ...
    # ---------------------------------------------------------------------
    # Claude Monitor (AI monitoring service)
    # ---------------------------------------------------------------------

    def _is_claude_monitor_running(self) -> bool:
        """Check if Claude monitor process is running."""
        try:
            pid_file = self.project_root / "logs" / "claude_monitor.pid"
            if pid_file.exists():
                try:
                    pid = int(pid_file.read_text().strip())
                    result = subprocess.run(
            ...
            # Fallback: check by process name
            result = subprocess.run(
                ["pgrep", "-f", "pearlalgo.claude_monitor.monitor_service"],
                capture_output=True,
                timeout=5,
            )
            return result.returncode == 0
    ...
    def get_claude_monitor_status(self) -> Dict[str, Any]:
        """Get Claude monitor service status."""
        running = self._is_claude_monitor_running()
        ...
        return {
            ...
            "message": "🟢 Claude Monitor is RUNNING" if running else "⚪ Claude Monitor is STOPPED",
        }

    async def start_claude_monitor(self, background: bool = True) -> Dict[str, Any]:
        """Start Claude Monitor Service."""
        script = self.scripts_dir / "lifecycle" / "start_claude_monitor.sh"
        logger.info("Starting Claude Monitor via Telegram command", extra={"background": background})

        if self._is_claude_monitor_running():
            status = self.get_claude_monitor_status()
            return {
                "success": True,
                "message": "✅ Claude Monitor already running",
                "details": f"PID: {status.get('pid')}" if status.get("pid") else "Process detected",
            }

        args = ["--background"] if background else []
        success, stdout, stderr = self._run_script(script, args=args, timeout=30, check=False)

        # Verify it started
        await asyncio.sleep(2)
        is_running = self._is_claude_monitor_running()

        if is_running:
            status = self.get_claude_monitor_status()
            return {
                "success": True,
                "message": "✅ Claude Monitor started successfully",
                "details": stdout.strip() if stdout else f"PID: {status.get('pid')}",
                "background": background,
            }

        return {
            "success": False,
            "message": "❌ Failed to start Claude Monitor",
            "details": (stderr.strip() if stderr else stdout.strip()),
        }

    async def stop_claude_monitor(self) -> Dict[str, Any]:
        """Stop Claude Monitor Service."""
        script = self.scripts_dir / "lifecycle" / "stop_claude_monitor.sh"
        logger.info("Stopping Claude Monitor via Telegram command")

        if not self._is_claude_monitor_running():
            return {
                "success": False,
                "message": "⚠️ Claude Monitor is not running",
                "details": "Nothing to stop",
            }
        ...
        if not is_running:
            return {
                "success": True,
                "message": "✅ Claude Monitor stopped successfully",
                "details": stdout.strip() if stdout else "Monitor process stopped",
            }

        return {
            "success": False,
            "message": "⚠️ Stop command executed but Claude Monitor still running",
            "details": stderr.strip() if stderr else stdout.strip(),
        }
