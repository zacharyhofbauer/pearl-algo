...
class PerformanceTracker:
    ...
    def track_entry(self, signal_id: str, entry_price: float, entry_time: Optional[datetime] = None, signal_data: Optional[Dict] = None) -> None:
        """
        Track signal entry.
    ...
    def track_exit(
        ...
        if not signal_record:
            ...
            return None
        if str(signal_record.get("status") or "").lower() == "exited":
    ...
    def _update_signal_status(self, signal_id: str, status: str, data: Dict, preserve_signal: Optional[Dict] = None) -> None:
        """
        ...
        if self._sqlite_enabled and self._trade_db is not None:
            ...
            try:
                ...
                # Use preserved signal if provided (avoids race condition)
                current_record: Optional[Dict] = preserve_signal
