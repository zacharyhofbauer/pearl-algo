"""
MFFU 50K Rapid Evaluation Tracker
...
Design:
  ...
  - MFFU-specific fields are nested under "mffu" key so the UI can detect the
    stage and render extended info.
"""
...
@dataclass
class MFFUEvalConfig:
    """MFFU 50K Rapid Evaluation configuration."""
...
@dataclass
class MFFUEvalAttempt:
    """Single MFFU evaluation attempt."""
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "MFFUEvalAttempt":
        return cls(
...
class MFFUEvaluationTracker:
    """
    Tracks MFFU 50K Rapid Evaluation challenge state.
    ...
    def __init__(
        self,
        config: Optional[MFFUEvalConfig] = None,
        state_dir: Optional[Path] = None,
    ):
        self.config = config or MFFUEvalConfig()
        ...
        logger.info(
            f"MFFUEvaluationTracker initialized: attempt={self.current_attempt.attempt_id}, "
    ...
    def _save_state(self, attempt: Optional[MFFUEvalAttempt] = None) -> None:
        ...
        try:
            ...
            logger.error(f"Could not save MFFU challenge state: {e}")

    def _save_to_history(self, attempt: MFFUEvalAttempt) -> None:
        try:
            ...
            logger.error(f"Could not save MFFU challenge history: {e}")
    ...
    def record_trade(
        ...
        if self.current_attempt.pnl >= self.config.profit_target:
            ...
            if consistency["met"] and min_days["met"]:
                ...
                logger.info(
                    f"MFFU profit target reached but cannot pass yet: "
    ...
    def update_eod_hwm(self, eod_balance: Optional[float] = None) -> None:
        ...
        self._save_state()
        logger.info(
            f"MFFU EOD HWM updated: balance=${eod_balance:.2f}, "
    ...
    def check_intraday_breach(self, current_equity: float) -> bool:
        ...
        if current_equity < self.current_attempt.current_drawdown_floor:
            logger.warning(
                f"MFFU INTRADAY BREACH: equity=${current_equity:.2f} < "
                f"floor=${self.current_attempt.current_drawdown_floor:.2f}"
            )
    ...
    def _end_attempt(self, outcome: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"MFFU attempt #{self.current_attempt.attempt_id} ended: "
    ...
    def refresh(self) -> None:
        ...
        try:
            ...
            logger.debug(f"Could not refresh MFFU state: {e}")
