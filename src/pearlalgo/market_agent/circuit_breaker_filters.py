...
def check_ml_chop_shield(
    ...
    if ml_stats is None:
        return CircuitBreakerDecision(allowed=True, reason="ml_chop_shield_no_stats")

    scored_trades = int(ml_stats.get("scored_trades", 0))
    if scored_trades < config.ml_min_scored_trades:
        return CircuitBreakerDecision(
            allowed=True,
            reason="ml_chop_shield_insufficient_data",
            details={"scored_trades": scored_trades, "required": config.ml_min_scored_trades},
        )
    ...
    if win_rate_delta < config.ml_min_winrate_delta:
        return CircuitBreakerDecision(
            allowed=True,
            ...
            details={
                ...
                "required_delta": config.ml_min_winrate_delta,
            },
        )

    ml_prediction = signal.get("_ml_prediction") or {}
    ml_pass = ml_prediction.get("pass_filter", True)
    if ml_pass:
        return CircuitBreakerDecision(
            allowed=True,
    ...
    shield_regimes_lower = [r.lower() for r in config.ml_chop_shield_regimes]
    if regime_type not in shield_regimes_lower:
        return CircuitBreakerDecision(
            allowed=True,
            reason="ml_chop_shield_regime_not_targeted",
            details={"regime": regime_type, "shield_regimes": config.ml_chop_shield_regimes},
        )
