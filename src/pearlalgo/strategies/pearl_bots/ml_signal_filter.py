"""
ML-Enhanced Signal Filter for Pearl Bots

Integrates with the existing contextual bandit system to enhance signal quality:
...
class MLSignalFilter:
    ...
    def __post_init__(self):
        ...
        if self.enabled:
            try:
                # Create contextual bandit policy with default config
    ...
    def enhance_signal(self, signal: "TradeSignal", df: pd.DataFrame) -> "TradeSignal":
        ...
        try:
            ...
            # Create context features for the bandit
            context = self._create_context_features(df, regime, regime_metrics)

            # Create signal dict for bandit
            signal_dict = self._signal_to_dict(signal)

            # Get bandit decision
            decision = self.bandit_policy.decide(signal_dict, context)

            # Apply ML enhancements to signal
            enhanced_signal = self._apply_ml_enhancements(signal, decision, regime, regime_confidence)
    ...
    def _create_context_features(self, df: pd.DataFrame, regime: MarketRegime,
                               regime_metrics: Any) -> ContextFeatures:
        ...
        vol_percentile = recent_volatility / historical_volatility if historical_volatility > 0 else 0.5

        # Time features (simplified - would need market hours integration)
        ...
        is_last_hour = minutes_since_open > 330  # 5.5 hours into session
    ...
    def _signal_to_dict(self, signal: TradeSignal) -> Dict[str, Any]:
        ...
        return {
            "type": f"pearl_{signal.bot_name.lower()}",
    ...
    def _apply_ml_enhancements(self, signal: "TradeSignal", decision: ContextualDecision,
                             regime: MarketRegime, regime_confidence: float) -> "TradeSignal":
        """Apply ML insights to enhance the signal."""
        enhanced_signal = signal

        # Adjust confidence based on bandit's expected win rate
        expected_win_rate = decision.expected_win_rate
        current_confidence = signal.confidence

        # Calculate confidence adjustment
        if expected_win_rate > 0.6:  # Good historical performance
            confidence_boost = min(self.confidence_boost_limit,
                                 (expected_win_rate - 0.5) * 0.5)
        elif expected_win_rate < 0.4:  # Poor historical performance
            confidence_boost = max(self.confidence_penalty_limit,
                                 (expected_win_rate - 0.5) * 0.5)
        else:
        ...
        enhanced_signal.regime_adjusted_confidence = np.clip(
            current_confidence + final_confidence_boost, 0.0, 1.0
        )

        # Add ML insights to features
        ...
        # Update reason to include ML insights
        if abs(final_confidence_boost) > 0.05:  # Significant adjustment
            boost_desc = "boosted" if final_confidence_boost > 0 else "reduced"
            enhanced_signal.reason += f" [ML {boost_desc} confidence by {abs(final_confidence_boost):.2f} based on {decision.context_sample_count} similar trades]"
    ...
    def _get_regime_confidence_multiplier(self, regime: MarketRegime, regime_confidence: float) -> float:
        ...
        # Higher confidence in stable regimes, lower in volatile/mixed
        multipliers = {
    ...
    def _record_decision(self, decision: ContextualDecision, signal_dict: Dict[str, Any],
                        context: ContextFeatures) -> None:
        """Record decision for future learning."""
        # This would be called after trade execution to update the bandit
        # For now, we just log the decision
        if decision.execute:
            logger.debug(f"ML Filter: Executing signal {signal_dict.get('type')} in {context.regime} regime "
                        f"(expected win rate: {decision.expected_win_rate:.2f})")
        else:
            logger.debug(f"ML Filter: Rejecting signal {signal_dict.get('type')} in {context.regime} regime "
                        f"(expected win rate: {decision.expected_win_rate:.2f})")
    ...
    def update_from_trade_result(self, signal: "TradeSignal", pnl: float, was_executed: bool) -> None:
        ...
        try:
            # Convert signal back to dict format
            signal_dict = self._signal_to_dict(signal)

            # Determine if trade was successful (positive P&L)
            success = pnl > 0

            # Update the bandit with the result
            # Note: This would need integration with the actual trade execution feedback
            logger.debug(f"ML Filter: Recording trade result for {signal.bot_name} - "
...
# Global ML signal filter instance
