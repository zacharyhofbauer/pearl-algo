...
def optimize_parameters(
    ...
    console.print(f"[cyan]Testing {len(param_combinations)} parameter combinations...[/cyan]")
...
def main() -> int:
    ...
    if args.optimize:
        ...
        if not param_grid:
            console.print("[yellow]No parameter grid specified. Using default optimization...[/yellow]")
        ...
        if result["results"]:
            ...
            table.add_row("Average Win Rate", f"{result['results']['avg_win_rate']:.1f}%")
        ...
        if not results:
            console.print("[red]No test windows generated. Check data and parameters.[/red]")
            return 1
        
        # Display results
        table = Table(title="Walk-Forward Test Results", box=box.ROUNDED, header_style="bold cyan")
        ...
        avg_win_rate = sum(r["win_rate"] for r in results) / len(results) if results else 0.0
