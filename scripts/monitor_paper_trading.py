...
def validate_risk_calculations(state: TradingState):
    ...
    if state.position_decisions:
        ...
        for symbol, decision in state.position_decisions.items():
            logger.info(
                f"   {symbol}: {decision.action} (size: {decision.size}, risk: {decision.risk_pct * 100:.2f}%)"
            )
    else:
    ...
    for symbol, decision in state.position_decisions.items():
        if decision.size > 0:
            ...
            if risk_pct > 0.02:  # 2% max
                logger.error(
                    f"❌ {symbol}: Risk exceeds 2% limit! ({risk_pct * 100:.2f}%)"
                )
                return False
            else:
        ...
        if config_path.exists():
            ...
            if telegram.get("enabled", False):
                ...
                if bot_token and chat_id:
                    try:
                        ...
                        result = asyncio.run(
                            alerter.send_message(
                                "🧪 Paper Trading Test - LangGraph System"
                            )
                        )
                        if result:
                    ...
                    logger.info(
                        "ℹ️  Telegram not configured (bot_token or chat_id missing)"
                    )
            else:
