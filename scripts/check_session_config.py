...
def check_config():
    ...
    table = Table(title="Session Settings", box=box.ROUNDED, show_header=True, header_style="bold cyan")
    ...
    port_type = "Paper Trading" if port == 4002 else "Live Trading" if port == 4001 else "Custom"
    ...
    session_status = "⚠️  Conflicts with mobile" if client_id <= 1 else "✅ Compatible with mobile"
    ...
    if client_id <= 1:
        console.print("[yellow]⚠️  Current Client ID ({}) may conflict with mobile app[/yellow]".format(client_id))
        ...
        console.print("[green]✅ Client ID {} is compatible with mobile app[/green]".format(client_id))
        console.print("   → Mobile app can use Client ID 0/1 (main session)")
        console.print("   → Automated system uses Client ID {} (secondary session)\n".format(client_id))
    
    if port == 4002:
        console.print("[green]✅ Port 4002 (Paper Trading) - Both can connect simultaneously[/green]\n")
    else:
        console.print("[yellow]⚠️  Port {} - Make sure this matches your paper trading setup[/yellow]\n".format(port))
