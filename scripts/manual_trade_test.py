...
def enter_trade(broker: IBKRBroker, portfolio: Portfolio) -> None:
    ...
    try:
        ...
        if order_id:
            ...
            console.print("[yellow]⚠️  Order may be in dry-run mode (check settings)[/yellow]\n")
...
def close_position(broker: IBKRBroker, portfolio: Portfolio) -> None:
    ...
    try:
        ...
        try:
            ...
            if order_id:
                console.print(f"[green]✅ Close order submitted successfully![/green]")
                console.print(f"  Order ID: {order_id}\n")
            else:
...
def test_connection():
    ...
    try:
        ...
        console.print(f"  Live Trading: {'✅ Enabled' if settings.allow_live_trading else '❌ Disabled (DRY RUN)'}\n")
        
        if not settings.allow_live_trading:
            console.print("[yellow]⚠️  WARNING: Live trading is disabled![/yellow]")
            console.print("[yellow]   Orders will be logged but not executed (DRY RUN mode)[/yellow]\n")
...
def main():
    ...
    while True:
        ...
        console.print("[bold cyan]║[/bold cyan]  [bold yellow]🧪 Manual Trading Test Menu[/bold yellow]                        [bold cyan]║[/bold cyan]")
        console.print("[bold cyan]╚═══════════════════════════════════════════════════════════╝[/bold cyan]\n")
        
        table = Table(show_header=False, box=box.SIMPLE, padding=(0, 2))
        table.add_row("[bold green]1.[/bold green]", "📊 Show Positions", "View current open positions")
