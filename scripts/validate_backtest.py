...
def compare_backtest_paper(
    ...
    if not backtest_results.empty and "realized_pnl" in backtest_results.columns:
        ...
        ) if len(backtest_trades) > 0 else 0.0
    ...
    if not paper_results.empty and "realized_pnl" in paper_results.columns:
        ...
        ) if len(paper_trades) > 0 else 0.0
    ...
    if abs(comparison["backtest_trades"] - comparison["paper_trades"]) > comparison["paper_trades"] * 0.2:
        comparison["discrepancies"].append(
...
def validate_signal_consistency(
    ...
    if "timestamp" not in backtest_signals.columns or "timestamp" not in paper_signals.columns:
        return {
    ...
    for _, backtest_sig in backtest_signals.iterrows():
        ...
        time_diff = abs((paper_signals["timestamp"] - backtest_sig["timestamp"]).dt.total_seconds())
    ...
    match_rate = (matched / len(backtest_signals) * 100) if len(backtest_signals) > 0 else 0.0
...
def main() -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Validate backtest results against paper trading")
    parser.add_argument(
        "--backtest",
        type=Path,
        required=True,
    ...
    # Compare results
    comparison = compare_backtest_paper(backtest_df, paper_df, tolerance_pct=args.tolerance)
    
    # Display comparison
    table = Table(title="Backtest vs Paper Trading Comparison", box=box.ROUNDED, header_style="bold cyan")
    ...
    if comparison["discrepancies"]:
        ...
        console.print(Panel(discrepancy_text, title="⚠️  Discrepancies Found", border_style="yellow"))
    else:
        console.print(Panel("✅ No significant discrepancies found", title="Validation", border_style="green"))
    
    # Signal consistency validation
    if args.signals:
        if not args.backtest_signals or not args.backtest_signals.exists():
            console.print("[yellow]Backtest signals file not provided or not found. Skipping signal validation.[/yellow]")
        elif not args.paper_signals or not args.paper_signals.exists():
            console.print("[yellow]Paper signals file not provided or not found. Skipping signal validation.[/yellow]")
        else:
            ...
            sig_table.add_row("Backtest Signals", str(signal_validation["backtest_signals"]))
            sig_table.add_row("Paper Signals", str(signal_validation["paper_signals"]))
            sig_table.add_row("Matched Signals", str(signal_validation["matched_signals"]))
            sig_table.add_row("Match Rate", f"{signal_validation['match_rate']:.1f}%")
            
            console.print(sig_table)
            
            if signal_validation["mismatches"]:
                console.print(f"[yellow]Found {len(signal_validation['mismatches'])} signal mismatches (showing first 10)[/yellow]")
