...
def check_positions(ib):
    ...
    try:
        ...
        console.print("[green]✅ Can access positions[/green]")
        console.print(
            f"   Open positions: {len([p for p in positions if p.position != 0])}\n"
        )
...
def check_contract_lookup(ib):
    ...
    try:
        ...
        if success_count == len(test_symbols):
            console.print("[green]✅ Contract lookup working[/green]")
            console.print(
                f"   Tested {len(test_symbols)} symbols: {success_count}/{len(test_symbols)} successful\n"
            )
            return True
        else:
            console.print("[yellow]⚠️  Contract lookup partial[/yellow]")
            console.print(
                f"   Tested {len(test_symbols)} symbols: {success_count}/{len(test_symbols)} successful\n"
            )
            return False
