...
def main():
    ...
    groq_model = (
        config.get("llm", {}).get("groq", {}).get("model", "llama-3.1-70b-versatile")
    )
    ...
    results["anthropic"] = test_provider(
    ...
    if results.get("anthropic"):
        asyncio.run(
            test_llm_reasoning("anthropic", "ANTHROPIC_API_KEY", anthropic_model)
        )
