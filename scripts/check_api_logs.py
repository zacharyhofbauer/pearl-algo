...
def check_logs():
    ...
    table = Table(title="Log Analysis", box=box.ROUNDED, show_header=True, header_style="bold cyan")
    ...
    console.print("[bold cyan]🔍 Searching for Order Execution Evidence...[/bold cyan]\n")
    ...
    for log_name, log_path in log_files:
        ...
        try:
            ...
            patterns = {
                "Order placed successfully": content.count("Order placed successfully") + content.count("OrderID="),
            ...
            if any(count > 0 for count in patterns.values()):
                ...
                for pattern, count in patterns.items():
                    if count > 0:
                        console.print(f"  [green]✓[/green] {pattern}: {count} occurrences")
                console.print()
...
def check_ibkr_connection_logs():
    ...
    if connection_evidence:
        ...
        console.print("[yellow]⚠️  No clear connection/execution evidence in logs[/yellow]\n")
...
if __name__ == "__main__":
    ...
    console.print("  - Check above for 'SUBMITTING LIVE ORDER' and 'Order placed successfully'")
