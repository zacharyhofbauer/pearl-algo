...
def compute_trade_statistics(perf_df: pd.DataFrame) -> dict[str, Any]:
    ...
    if perf_df.empty:
        return {
            ...
            "avg_hold_time_minutes": 0.0, "largest_winner": 0.0, "largest_loser": 0.0,
    ...
    return {
        "total_trades": total_trades, "winners": len(winners), "losers": len(losers),
        "win_rate": win_rate, "avg_hold_time_minutes": avg_hold_time,
...
def aggregate_pnl_by_symbol(perf_df: pd.DataFrame) -> dict[str, dict[str, float]]:
    ...
    for symbol in perf_df["symbol"].unique():
        ...
        result[str(symbol)] = {"realized": float(realized), "unrealized": float(unrealized)}
...
def extract_signal_context(perf_df: pd.DataFrame, signals_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for symbol in perf_df["symbol"].unique():
        ...
        for idx, row in sym_perf.head(3).iterrows():
            ...
            context_row = {
                "symbol": str(symbol), "strategy": row.get("strategy_name", "unknown"),
                "direction": signal_row.get("direction", row.get("side", "FLAT")).upper() if signal_row is not None else row.get("side", "FLAT").upper(),
                "entry_price": row.get("entry_price"), "stop_price": None, "target_price": None,
                "vwap": sr_dict.get("vwap"), "support1": sr_dict.get("support1"), "resistance1": sr_dict.get("resistance1"),
                ...
                "confidence": None, "timestamp": row.get("timestamp"),
            }
...
def get_decision_reasoning(perf_df: pd.DataFrame, signals_df: pd.DataFrame) -> dict[str, Any]:
    ...
    today_df = perf_df[perf_df["timestamp"].dt.strftime("%Y%m%d") == today] if "timestamp" in perf_df.columns else pd.DataFrame()
    
    for symbol in perf_df["symbol"].dropna().unique():
        sym_df = today_df[today_df["symbol"] == symbol] if not today_df.empty else pd.DataFrame()
        if sym_df.empty:
            continue
...
def create_header_panel(processes: list[dict[str, Any]], refresh_interval: int, seconds_until_refresh: float) -> Panel:
    ...
    content.append(f"UTC: {now_utc.strftime('%Y-%m-%d %H:%M:%S')} | ET: {now_eastern}\n", style="dim")
    ...
    if processes:
        ...
        if all_symbols:
            ...
            content.append(f"Symbols: {', '.join(sorted(set(all_symbols)))} | ", style="yellow")
            content.append(f"Strategies: {', '.join(sorted(all_strategies))}", style="cyan")
    else:
        content.append("\n\n", style="dim")
...
def create_decision_reasoning_panel(perf_df: pd.DataFrame, signals_df: pd.DataFrame) -> Panel:
...
def create_recent_trades_panel(perf_df: pd.DataFrame) -> Panel:
    ...
    if perf_df.empty:
        return Panel("[dim]No trades yet[/dim]", title="📝 Recent Trades", border_style="yellow")
...
def analyze_why_no_trades(perf_df: pd.DataFrame, signals_df: pd.DataFrame, profile: Any) -> Panel:
    ...
    if flat_count > 0:
        content.append(f"📊 Signal Analysis:\n", style="bold")
    ...
    if risk_state.status != "OK":
        ...
        if risk_state.status == "HARD_STOP":
            ...
            content.append(f"  Cooldown active until {risk_state.cooldown_until.strftime('%H:%M:%S') if risk_state.cooldown_until else 'N/A'}\n", style="yellow")
        elif risk_state.status == "NEAR_LIMIT":
            content.append("  Approaching risk limits - sizing reduced\n", style="yellow")
...
def create_dashboard(refresh_interval: int = 60, seconds_until_refresh: float = 60.0, show_menu: bool = False) -> Layout:
    ...
    # Footer
    footer_text = Text("Press Ctrl+C to exit | Refresh: " + str(refresh_interval) + "s", style="dim", justify="center")
