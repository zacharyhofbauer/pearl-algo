...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Simple risk monitor loop (daily loss guard)."
    )
    parser.add_argument(
        "--max-daily-loss", type=float, required=True, help="Trigger halt if breached."
    )
    parser.add_argument(
        ...
        while True:
            ...
            if realized < -abs(args.max_daily_loss):
                halt_file.write_text(
                    f"{ts} Breached max daily loss {args.max_daily_loss}, pnl={realized}\n"
                )
                print(
                    f"[{ts}] RISK HALT: pnl={realized} < -{args.max_daily_loss}. Wrote {halt_file}"
                )
                break
            if halt_file.exists():
