...
def show_status():
    ...
    console.print("\n[bold cyan]╔═══════════════════════════════════════════════════════════╗[/bold cyan]")
    console.print("[bold cyan]║[/bold cyan]  [bold yellow]🎯 PearlAlgo Futures Desk — System Status[/bold yellow]            [bold cyan]║[/bold cyan]")
    ...
    table.add_row("Port 4002", port_icon, "Listening" if port_listening else "Not listening")
    
    api_icon = "✅" if api_ready else "⏳"
    table.add_row("API Ready", api_icon, "Accepting connections" if api_ready else "Not ready yet")
    ...
    config_table.add_row("IBC Config", "✅" if ibc_config.exists() else "❌", str(ibc_config))
    
    venv_path = PROJECT_ROOT / ".venv"
    config_table.add_row("Virtual Env", "✅" if venv_path.exists() else "❌", str(venv_path))
    
    perf_path = PROJECT_ROOT / "data" / "performance" / "futures_decisions.csv"
    config_table.add_row("Perf Log", "✅" if perf_path.exists() else "⚠️ ", str(perf_path))
...
def setup_wizard():
    ...
    console.print("\n[bold cyan]╔═══════════════════════════════════════════════════════════╗[/bold cyan]")
    console.print("[bold cyan]║[/bold cyan]  [bold yellow]🎯 PearlAlgo Futures Desk — Setup Wizard[/bold yellow]         [bold cyan]║[/bold cyan]")
    console.print("[bold cyan]╚═══════════════════════════════════════════════════════════╝[/bold cyan]\n")
    ...
    if venv_path.exists():
        ...
        if Confirm.ask("Create virtual environment?"):
            ...
            console.print("[yellow]   Run: source .venv/bin/activate && pip install -e .[/yellow]\n")
            return
    console.print()
    ...
    if is_running:
        ...
        if check_gateway_port():
            ...
            if check_gateway_api():
                ...
                console.print("[yellow]⏳ API not ready yet (may need more time)[/yellow]")
                if Confirm.ask("Wait for API to be ready?"):
                    wait_for_gateway()
        else:
            console.print("[yellow]⚠️  Port 4002 not listening (Gateway may still be starting)[/yellow]")
    else:
    ...
    if check_gateway_api():
        ...
        console.print("[bold]You're all set! Run: [cyan]python scripts/workflow.py[/cyan][/bold]\n")
    else:
        console.print("[yellow]⚠️  Connection test failed[/yellow]")
        console.print("[yellow]   Make sure Gateway is running and API is enabled[/yellow]\n")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="PearlAlgo Futures Desk — Setup & Management Assistant")
    parser.add_argument("--status", action="store_true", help="Show system status")
    parser.add_argument("--setup", action="store_true", help="Run setup wizard")
    parser.add_argument("--quick-start", action="store_true", help="Quick start (ensure Gateway is running)")
    ...
    parser.add_argument("--restart-gateway", action="store_true", help="Restart IB Gateway")
    parser.add_argument("--wait-gateway", action="store_true", help="Wait for Gateway to be ready")
    parser.add_argument("--test-connection", action="store_true", help="Test IBKR API connection")
    ...
    if args.status:
        ...
        if check_gateway_api():
            ...
            console.print("[yellow]   Make sure Gateway is running: python scripts/setup_assistant.py --start-gateway[/yellow]\n")
    else:
        # Interactive menu
        while True:
            console.print("\n[bold cyan]╔═══════════════════════════════════════════════════════════╗[/bold cyan]")
            console.print("[bold cyan]║[/bold cyan]  [bold yellow]🎯 PearlAlgo Setup & Management Assistant[/bold yellow]    [bold cyan]║[/bold cyan]")
            console.print("[bold cyan]╚═══════════════════════════════════════════════════════════╝[/bold cyan]\n")
            
            table = Table(show_header=False, box=box.SIMPLE, padding=(0, 2))
            table.add_row("[bold green]1.[/bold green]", "📊 Show System Status", "Check Gateway, config, and connections")
            table.add_row("[bold green]2.[/bold green]", "🔧 Run Setup Wizard", "Interactive setup guide")
            table.add_row("[bold green]3.[/bold green]", "🚀 Quick Start", "Ensure Gateway is running and ready")
            table.add_row("[bold green]4.[/bold green]", "▶️  Start IB Gateway", "Start Gateway in background")
            table.add_row("[bold green]5.[/bold green]", "⏹️  Stop IB Gateway", "Stop running Gateway")
            table.add_row("[bold green]6.[/bold green]", "🔄 Restart IB Gateway", "Stop and start Gateway")
            table.add_row("[bold green]7.[/bold green]", "⏳ Wait for Gateway", "Wait until API is ready")
            table.add_row("[bold green]8.[/bold green]", "🔍 Test Connection", "Test IBKR API connection")
