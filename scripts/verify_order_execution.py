    ...
    try:
        ...
        if all_trades:
            ...
            for trade in all_trades[-20:]:
                ...
                order_status = (
                    trade.orderStatus if hasattr(trade, "orderStatus") else None
                )
        ...
        if fills:
            ...
            for fill in fills[-10:]:  # Last 10 fills
                ...
                contract = (
                    exec_report.contract if hasattr(exec_report, "contract") else None
                )
                symbol = (
                    contract.symbol
                ...
                fill_table.add_row(
                    symbol, side, str(int(shares)), f"${price:,.2f}", time_str
                )
        ...
        filled_orders = [
            t
            for t in all_trades
            if hasattr(t, "orderStatus") and t.orderStatus.status == "Filled"
        ]
        cancelled_orders = [
            t
            for t in all_trades
            if hasattr(t, "orderStatus") and t.orderStatus.status == "Cancelled"
        ]
        ...
        console.print(
            f"  ⏳ Open Orders: {len([t for t in all_trades if hasattr(t, 'orderStatus') and t.orderStatus.status in ['Submitted', 'PreSubmitted']])}"
        )
        console.print()

        if filled_orders:
            console.print(
                "[bold green]✅ SUCCESS: Orders are being executed![/bold green]\n"
            )
        elif cancelled_orders:
            console.print(
                "[bold yellow]⚠️  Orders are being cancelled - check error messages above[/bold yellow]\n"
            )
        else:
