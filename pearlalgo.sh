...
restart_services_only() {
    ...
    echo -e "${CYAN}Starting dev webapp...${NC}"
    sudo systemctl start pearlalgo-webapp-dev 2>/dev/null || true
    sleep 1

    # Status
    echo ""
    all_ok=true
    for svc in pearlalgo-agent pearlalgo-api pearlalgo-webapp pearlalgo-webapp-dev; do
