...
- Reusable AI prompts are stored in `prompts/` directory
...
- ❌ Implemented but incorrect or fragile
...
### 3. File-Level Action Plan (Table Format)

| File Path | Action | Target (if merge) | Reason | Risk Level | Verification |
|-----------|--------|-------------------|--------|------------|--------------|
| `path/to/file.py` | DELETE | — | Unreferenced, no imports | Low | `grep -r "file" src/ scripts/ tests/` |
| `utils/old_helper.py` | MERGE | `utils/new_helper.py` | Duplicate logic, newer version exists | Medium | Check call sites |
...
| Responsibility | Current Owner(s) | Recommended Owner | Status |
|----------------|------------------|-------------------|--------|
| State persistence | `state_manager.py` | ✅ `state_manager.py` | Clear |
| Error handling | `utils/error_handler.py`, `service.py` | ⚠️ Needs consolidation | Ambiguous |
