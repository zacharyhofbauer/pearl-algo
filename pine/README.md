...
| File | What it does |
|---|---|
| `pearl_algo_crypto/pearl_algo_crypto_v5.pine` | **Crypto/perp v5 (Pine v6).** Forks the working MNQ Hourly v5 engine/dashboard, then adapts defaults for crypto day-trading: 15m execution / 4h confirmed regime (both read via anti-repaint `request.security` at the effective chart-or-higher TF, so it renders and fires on ANY chart timeframe — view on 1h/4h and the signal auto-raises rather than reading degraded sub-chart data), 24x7 Auto sessions for crypto/CRYPTOCAP/USDT/perp-like tickers, percent taker costs, MNQ-style top-right dashboard, basis/funding-window/relative-volume perp risk rows, and `pearl-algo-crypto-v5` JSON alerts. **RESEARCH — alerts MUTED by default; validate per symbol/timeframe before leverage.** Current crypto script — supersedes the `pearl_algo_crypto_v1`–`v4` research snapshots kept in the folder for history. |
| `pearl_universal_defender.pine` | **Universal higher-timeframe PEARL (Pine v6).** Symbol-agnostic version of the PEARL chart kit for MNQ, other futures, crypto spot, and crypto perps. Defaults to 4h execution / 1D regime, confirmed `request.security` reads, selective confluence, Auto RTH-vs-24x7 session handling, flexible qty/max-qty inputs, MNQ v5-style dashboard/overlays, perp basis/funding-window/volume-stress filters, and `pearl-universal-defender` JSON alerts. **RESEARCH — alerts MUTED by default; validate per market/timeframe before leverage.** |
| `btc_trend_defender.pine` | **BTC research candidate (Pine v6).** 1h chart / 4h confirmed regime / daily 200 EMA filter / long-only Donchian breakout with wide Chandelier exit. Defaults come from a bounded 72-variant disconfirmation sweep on Coinbase BTC-USD hourly data: Donchian 100, ADX 10, 8x ATR trail, shorts OFF, alerts MUTED. Local backtest 2019-07-05->2026-07-01: 100 trades, +282.09%, PF 1.662, max DD -36.56%, but bootstrap CI crosses zero, so this is paper/research only. |
